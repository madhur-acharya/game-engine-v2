...
export class Rectangle{
	...
	Start= () => {
		...
		/*this.gameObject.addComponent(new VGRenderer(obj => {
			drawRectangle(this.gameObject.position, 32, 32, this.color);
		}));*/
		this.gameObject.addComponent(new SpriteRenderer(tileSet2, 0, 0, 16, 16, 32, 32));

		this.rb= this.gameObject.addComponent(new RigidBody(false, false));
		this.rb.kinematic= Math.round(Math.random() * 10) % 2 ? true : false;
		this.gameObject.addComponent(new Collider("box", {width: 32, height: 32}, () => {console.log("!")}));
	}
